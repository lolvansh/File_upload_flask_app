...
function Login({ onLogin }){
    ...
    return (
        <div className="flex flex-col items-center justify-center min-h-[60vh]">
            <div className="backdrop-blur-sm bg-blue-50 p-8 rounded-2xl border border-black/10 shadow-2xl shadow-sky-100/50 w-full max-w-md">
                <h2 className="text-3xl font-bold text-slate-800 text-center mb-2">
                    {isRegistering ? (otpSent ? "Verify Email" : "Create Account") : "Welcome Back"}
                </h2>
                <p className="text-slate-500 text-center mb-8">
                    {isRegistering 
                        ? (otpSent ? `Code sent to ${email}` : "Join your private cloud gallery") 
                        : "Enter your details to access files"}
                </p>

                <form onSubmit={(e) => e.preventDefault()} className="space-y-6" onKeyDown={handleKeyPress}>
                    
                    {/* STEP 1: Email & Password (Hide when OTP is sent) */}
                    {!otpSent && (
                        <>
                    ...
                    {/* STEP 2: OTP Input (Only show when OTP is sent) */}
                ...
                {/* Social Login / Toggle Login Mode */}
                {!otpSent && (
                    <>
